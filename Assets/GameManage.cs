...
using UnityEngine.SceneManagement; // Unity Scene 이동
	...
	public static void useItem(int x, int y) {
		...
		for (int i = 0; i < mapInfo.mapHeight; i++) {
			for (int j = 0; j < mapInfo.mapWidth; j++) {
				// isItemTargetable을 false로 초기화
				// null인 경우에 오류가 생기기 때문에 걸러줌
				if (lands[i, j] != null) {
		...
		if (editMode == EditMode.deleteBlock) {
			...
			for (int i = 0; i < mapInfo.mapHeight; i++) {
				for (int j = 0; j < mapInfo.mapWidth; j++) {
					...
					if (landType != LandType.air) {
						if (
							(editMode != EditMode.makePlayer && mapInfo.getLandType(i, j) == LandType.player)
